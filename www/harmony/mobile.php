...
$Functions = new DatabaseClasses;
    ...
    if (isset($_GET['do-resume'])){
        $data = $_POST['data'];
        $fname = 'resume.pdf';
        $file = fopen('../img/resume/'.$fname, 'w+');
        fwrite($file, $data[1]);
        fclose($file);

        $query = $Functions->PDO("UPDATE tbl_applicant SET resume ='{$fname}' WHERE id = '{$data[0]}'");
        // print_r($data[1]);
        if($query->execute()){
            echo 1;
        }
    ...
    if (isset($_GET['get-resume'])){
        $data = $_POST['data'];
        $query = $Functions->PDO("SELECT resume FROM tbl_applicant WHERE id = '{$data}'");
        print_r(json_encode($query));
    }
